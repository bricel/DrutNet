...
namespace SimplePDM
{
   ...
   public class ThumbnailCreator : IDisposable
   {
      ...
      private System.Drawing.Size desiredSize = new
      ...
      {
         if ((!BaseSimplePDM.FileExists(file)) && (!Directory.Exists(file)))
         {
         ...
         )
      {
         ...
         try
         {
            ...
            if
             (Path.GetFileName(pidlPath).ToUpper().Equals(Path.GetFileName(file)
            .ToUpper()))
            {
               // we have the item:                        
               IUnknown iunk = null;
               ...
               if (extractImage != null)
               {
                  ...
                  EIEIFLAG flags = EIEIFLAG.IEIFLAG_ASPECT |
                   EIEIFLAG.IEIFLAG_SCREEN;
